...
- Scope: Pull latest `origin/master`, merge remote direct-provider/runtime updates into the local Mastra branch, and assess remaining gap to the target architecture.
- Task type: `Code + Integration`
- Main updates:
- Created local safety branch `codex/backup-before-origin-merge-20260416` before integrating remote changes.
- Merged remote commits `bc350e9` and `5ae34b3` into the local branch without pushing.
- Resolved merge conflicts across BFF config/gateway/server, WebUI auth/mail contexts, markdown utilities, package metadata, and lockfile generation.
- Kept Mastra as the active primary agent runtime while absorbing remote direct-provider work:
  - retained `apps/bff/src/agent/*` Mastra runtime as the active `/api/agent/chat` and `/api/agent/query` path.
  - initialized direct Composio MCP config when `COMPOSIO_API_KEY + COMPOSIO_MCP_URL` are present.
  - preserved remote SiliconFlow direct fallback and Microsoft Graph direct Outlook support.
  - kept remote lightweight `apps/bff/src/runtime/*` files in-tree for comparison/fallback research, but not as the active server runtime.
- Resolved Windows case collision between `SUMMARY.md` and remote `summary.md` by keeping existing `SUMMARY.md` as the canonical project summary and not adding a duplicate lowercase file.
- Extended the Mastra runtime input contract so `/api/mail/query` can pass timezone, limit, horizon, and priority-rule context into mail tools.
- Verification completed:
- `npm --workspace apps/bff run check` passed.
- `npm --workspace apps/webui run check` passed.
- `npm --workspace apps/bff run build` passed.
